    ...
    while True:
        ...
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                system_instruction="You are a product information analyzer. Extract and return only  single string in format brandname modelname as it in user query name from the user's query. If the query is not about a product, return 'invalid query'. Format: 'BRAND: brand_name, MODEL: model_name' or 'invalid query'"),
            contents=user_input
        )
        
        result = response.text.strip()
        if "invalid query" in result.lower():
            print("\nI'm designed to help with product queries.")
            print("Please enter a query in the format: 'what is cost of [brand] [model]'")
            print("For example: 'what is cost of hp pavilion plus'")
        else:
             product_name=result
